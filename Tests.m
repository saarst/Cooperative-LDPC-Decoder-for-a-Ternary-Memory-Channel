...
% ratios = "u" + ["10"];
% sequences = {[4 2], [6 2]};
% rates = {[0.5, 0.4], [0.4, 0.3]}; % first is ind, second is res
% T = combinations(ratios, sequences, rates);
T = load("testLowRates.mat","T").T;
...
for i=1:height(T)
    ...
    RunPBS(experimentName, -8:0.5:-3, sequence(1), sequence(2), ratio, n, rate(1), rate(2), str2double(numIter), 2000)
end
